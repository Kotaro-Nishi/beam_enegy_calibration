...
SaveSharedP = "\SharedP_20210401_825_825_seed10000_new_zpfit_Yscreen_x"
...
for Saves in range(0,45):
    ...
    for boot in range(1):
        ...
        for i in range(NumOfSeries):
            listOfboots=list(range(NumOfSeries))#listOfboots.append(random.randint(0,LengthOfInterval-1))
        ...
        Guessvalueslmfit = [1.5,.0001,352.4,.190+.01+k*.001,.6,5.2,11.,1.]
    ...
    residsqrlststd = np.std(residsqrlst)
    ...
    with open(DataPath + SaveSharedP + "\\Residualstd.csv", 'a+', newline='') as write_obj:
        # Create a writer object from csv module
        csv_writer = csv.writer(write_obj)
        # Add contents of list as last row in the csv file
        csv_writer.writerow([residsqrlststd])        

    UnDi.SingleAmplitudeDone = 0

    if 0:#1: #do if image needed
