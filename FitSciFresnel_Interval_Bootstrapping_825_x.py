...
def diffraction_dataset(params,i,yy):
    ...
    Diff_intensity = UnDi.diffractionintensityInitialFit_X(yy,zetaxx,Ytran,i,AUndu.z0,WLofPixel,LambdaRfit,delta0fit+i*.001,gammafit,Amplfit,bfit,zpfit,Yscreenfit,13.,whichdata,.65)
...
for Saves in range(0,500):
    ...
    Guessvalueslmfit = [1.4,.0001,352.4,.190+.01+k*.001,.6,5.2]#,3.e-4]
    ...
    for Strpidx in range(NumOfSeries):
        ...
        y_fitLst.append(UnDi.diffractionintensity(zetaprime2,Ytran,listOfboots[Strpidx],AUndu.z0,WLofPixel,LambdaRfit,delta0fit+listOfboots[Strpidx]*.001,gammafit,Amplfit,bfit,zpfit,Yscreenfit)[500])
        dataLst.append((dataBooted[Strpidx])[500])
    ...
    for Strpidx in range(NumOfSeries):
        if 1:#Strpidx > 600 and Strpidx > 610:
            ...
            Sourcelst = UnDi.plainintensity(zetaprime2,Ytran,listOfboots[Strpidx],AUndu.z0,WLofPixel,LambdaRfit,delta0fit+.001*listOfboots[Strpidx],gammafit,Amplfit,bfit,zpfit,Yscreenfit)

            SourcelstIntensity1D = 55000.*abs(np.array(Sourcelst))

            y_fit = UnDi.diffractionintensity(zetaprime2,Ytran,listOfboots[Strpidx],AUndu.z0,WLofPixel,LambdaRfit,delta0fit+listOfboots[Strpidx]*.001,gammafit,Amplfit,bfit,zpfit,Yscreenfit)
