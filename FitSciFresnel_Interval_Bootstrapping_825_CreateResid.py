...
for Saves in range(0,500):
    ...
    Guessvalueslmfit = [1.4,.0001,352.4,.190+.01+k*.001,.6,5.45*.95]#,3.e-4]
    ...
    for nicestfit in range(20):
        ...
        for Strpidx in range(NumOfSeries):
            ...
            y_fitLst.append(UnDi.diffractionintensity(zetaprime2,Ytran,listOfboots[Strpidx],AUndu.z0,WLofPixel,LambdaRfit,delta0fit+listOfboots[Strpidx]*.001,gammafit,Amplfit,bfit,zpfit,Yscreenfit)[300+nicestfit*20])
            dataLst.append((dataBooted[Strpidx])[300+nicestfit*20])
    ...
    for Strpidx in range(NumOfSeries):
        if Strpidx == 100 or Strpidx == 225 or Strpidx == 350 or Strpidx == 475:#Strpidx > 600 and Strpidx > 610:
