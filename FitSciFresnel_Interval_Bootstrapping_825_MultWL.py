...
for ii in range(LengthOfInterval):
    ...
    """
    print(len(ListPictureMinusBGWidthOfStripe))
    
    if ii > 20:
        cv2.imshow("fdsa",PictureMinusBG)
        fntsze = 13
        ...
        #axA.plot(range(len(PictureMinusBGForSigma[0])), PictureMinusBGForSigma[0], 'r.')
        axA.plot(range(len(np.transpose(ListPictureMinusBGWidthOfStripeSigma)[100])), np.transpose(ListPictureMinusBGWidthOfStripeSigma)[100], 'r.')
...
SaveSharedP = "\SharedP_20210401_825_seed10000_MultiWL"
...
for Saves in range(0,NumOfStripes):
    ...
    for ii in range(LengthOfInterval):
        ...
        stripe = nptp(np.array([stripejjj])*6700000.)
    ...
    Guessvalueslmfit = [1.4,.0001,352.4,.190+.01,.6,4.3]#,3.e-4]
