...
NumOfStripes = 1
NumOfSeries = 824#819#825
LengthOfInterval = 824#819#825
...
Yslices = 8
Ygrid = int(LengthOfInterval/Yslices)
YtranLOWlst = []
YtranHIGHlst = []

for Yi in range(Yslices):
    YtranLOWlst.append(Ygrid*Yi)
    YtranHIGHlst.append(Ygrid*(Yi+1))
print(YtranLOWlst)
print(YtranHIGHlst)
...
Yscreenfit = (580)*10**-6 # Correct for D:\Doktor\BeamtimeFeb2020_Common\data\Beam20200320_1\Energymeasurement_1\raw_shortExp # Change guess gamma to 350
Yscreenfit = (300)*10**-6 # Correct for D:\Doktor\BeamtimeFeb2020_Common\data\Beam20200314_1\Energymeasurement_2\raw_shortExp # Change guess gamma to 380
#Yscreenfit = (460)*10**-6
#Yscreenfit = (260)*10**-6
LambdaRfit = 412e-9
...
def diffraction_dataset(params,i,yy):
    ...
    for Yi in range(Yslices):
        if i in list(range(YtranLOWlst[Yi],YtranHIGHlst[Yi])):
...
whichdata = 1 # Is NoConvolveData_825_SplitYTran.py
...
for Saves in range(0,1):
    ...
    SingularitemIntervallst = []
    for Yi in range(Yslices):
        for SIt in range(len(listOfboots[YtranLOWlst[Yi]:YtranHIGHlst[Yi]])):
            if listOfboots[YtranLOWlst[Yi]:YtranHIGHlst[Yi]].count(listOfboots[YtranLOWlst[Yi]:YtranHIGHlst[Yi]][SIt]) == 1:
                SingularitemIntervallst.append(SIt+Yi*Ygrid)
                break
    ...
    for iy,y, in enumerate(dataBooted):
        ...
        fit_params.add('Ytran_%i' % (listOfboots[iy]+iy*.000001),value=Guessvalueslmfit[1])#, min = Guessvalueslmfit[1]-.0001, max=Guessvalueslmfit[1]+.0001)
        ...
        fit_params.add('cfit_%i' % (listOfboots[iy]),value=Guessvalueslmfit[4])#, min = Guessvalueslmfit[4]-.1, max=Guessvalueslmfit[4]+.1)
        ...
        #fit_params.add('bfit_%i' % (listOfboots[iy]),value=Guessvalueslmfit[7])#, min = Guessvalueslmfit[4]-.1, max=Guessvalueslmfit[4]+.1)
    ...
    #listOfbootsnoFirstYtran = [x for x in listOfboots if x != listOfboots[SingularitemInteval1] and x != listOfboots[SingularitemInteval2] and  x != listOfboots[SingularitemInteval3] and  x != listOfboots[SingularitemInteval4])]
    listOfbootsnoFirstYtran = [x for x in listOfboots if x not in SingularitemIntervallst]
    #print(SingularitemIntervallst)
    #print(listOfbootsnoFirstYtran)
    for iy in tuple(range(len(listOfbootsnoFirstYtran))):#tuple(range(NumOfSeries)[1:]):
        for Yi in range(Yslices):
            if listOfbootsnoFirstYtran[iy] in list(range(YtranLOWlst[Yi],YtranHIGHlst[Yi])):
                fit_params['Ytran_%i' % listOfbootsnoFirstYtran[iy]].expr='Ytran_%i'% listOfboots[SingularitemIntervallst[Yi]]
                #print('Ytrun_%i'% listOfboots[SingularitemInteval1])
    ...
    for Yi in range(Yslices):
        YtranLst.append(result.params['Ytran_%i'% listOfboots[SingularitemIntervallst[Yi]]].value)
    YtranLst = np.array(YtranLst)

    print("YtranLst: ",YtranLst)
    print("std of YtranLst: ",np.std(YtranLst))
    print("std/sqrt(N) of YtranLst: ",np.std(YtranLst)/np.sqrt(Yslices))
    print("mean of YtranLst: ",np.mean(YtranLst))
    ...
    for Yi in range(len(SingularitemIntervallst)):
        Amplfit = result.params['Amplfit_%i'% listOfboots[FirstSingularitem]].value
        Ytran = result.params['Ytran_%i'% listOfboots[SingularitemIntervallst[Yi]]].value
        ...
        y_fit = UnDi.diffractionintensityInitialFit_X(zetaprime2,zetaxx,Ytran,listOfboots[SingularitemIntervallst[Yi]]+int(Ygrid/2),AUndu.z0,WLofPixel,LambdaRfit,delta0fit+(listOfboots[SingularitemIntervallst[Yi]]+int(Ygrid/2))*.001,gammafit,Amplfit,bfit,zpfit,Yscreenfit,elevationfit,whichdata,cfit)
        ...
        axA.plot(1E6*yyy[0:1200]/AUndu.z0, dataBooted[SingularitemIntervallst[Yi]+int(Ygrid/2)], 'b-', 1E6*yyy/AUndu.z0, y_fit, 'r-')
    ...
    for Yi in range(len(SingularitemIntervallst)):
        ...
        #axA.annotate(r'$\Delta d + d = %d $' % k, xy=(0, maxofplt), fontsize=fntsze)
        axA.set_title("Data and Fit and resid")
        axA.plot(1E6*yyy/AUndu.z0, y_fit)
    ...
    for Yi in range(len(SingularitemIntervallst)):
        ...
        y_fit = UnDi.diffractionintensityInitialFit_X(zetaprime2,zetaxx,Ytran,listOfboots[SingularitemIntervallst[Yi]]+int(Ygrid/2),AUndu.z0,WLofPixel,LambdaRfit,delta0fit+listOfboots[0]*.001,gammafit,Amplfit,bfit,zpfit,Yscreenfit,elevationfit,whichdata,cfit)
    ...
    for Yi in range(len(SingularitemIntervallst)):
        ...
        #axA.annotate(r'$\Delta d + d = %d $' % k, xy=(0, maxofplt), fontsize=fntsze)
        axA.set_title("Data and Fit and resid")
        axA.plot(1E6*yyy[0:1200]/AUndu.z0, dataBooted[listOfboots[SingularitemIntervallst[Yi]]+int(Ygrid/2)],)
