...
#print(type(pixelof404nm))

MoFu.pixelof404nm = MoFu.pixelof404nm/WidthOfStripe
CalibWl = 404.6565e-9
...
if mode == "posi":
    boot = 1
    MoFu.ModFit(Guessvalueslmfit,zetaprime2,List825VolumeMinusBG4Mean,WidthOfStripe,MoFu.pixelof404nm,MoFu.dispersion,CalibWl,NumOfSeries,LengthOfData,Interval,boot,[263],list(range(100))) ##Position variants

if mode == "lamb":
    boot = 0
    MoFu.ModFit(Guessvalueslmfit,zetaprime2,List825VolumeMinusBG4Mean,WidthOfStripe,MoFu.pixelof404nm,MoFu.dispersion,CalibWl,NumOfSeries,LengthOfData,Interval,boot,list(range(1,460,1)),[0]) ##WL variants

if mode == "Yscr":
    boot = 0
    Ylist = np.linspace(-300E-6+MoFu.Yscreenfit,300E-6+MoFu.Yscreenfit,16)
    for i in range(len(Ylist)):
        MoFu.Yscreenfit = Ylist[i]
        MoFu.ModFit(Guessvalueslmfit,zetaprime2,List825VolumeMinusBG4Mean,WidthOfStripe,MoFu.pixelof404nm,MoFu.dispersion,CalibWl,NumOfSeries,LengthOfData,Interval,boot,[263],[0]) ##Yscreenfit variants

if mode == "delt":
    boot = 0
    deltlist = np.linspace(-.05+Guessvalueslmfit[3],.05+Guessvalueslmfit[3],30)
    for i in range(len(deltlist)):
        Guessvalueslmfit[3] = deltlist[i]
        MoFu.ModFit(Guessvalueslmfit,zetaprime2,List825VolumeMinusBG4Mean,WidthOfStripe,MoFu.pixelof404nm,MoFu.dispersion,CalibWl,NumOfSeries,LengthOfData,Interval,boot,[263],[0]) ##delta0fit variants
