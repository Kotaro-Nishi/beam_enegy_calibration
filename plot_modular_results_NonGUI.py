...
for nicestfit in range(1):
    ...
    for Strpidx in range(int(NumOfSeries/everynth)):
        ...
        y_fitLst.append(UnDi.diffractionintensityInitialFit_X(zetaprime2,zetaxx,Ytran,listOfboots[Strpidx*everynth],AUndu.z0,WLofPixel,LambdaRfit,delta0fit+posdatas[listOfboots[Strpidx]]*.001,gammafit,Amplfit,bfit,zpfit,Yscreenfit,elevationfit,whichdata,cfit)[500])
...
for Strpidx in range(int(NumOfSeries/everynth)):
    if 1:#Strpidx == 100 or Strpidx == 225 or Strpidx == 350 or Strpidx == 475:#Strpidx > 600 and Strpidx > 610:
        ...
        Sourcelst = UnDi.plainintensity(zetaprime2,zetaxx,Ytran,listOfboots[Strpidx*everynth],AUndu.z0,WLofPixel,LambdaRfit,delta0fit+posdatas[listOfboots[Strpidx]]*.001,gammafit,Amplfit,bfit,zpfit,Yscreenfit,elevationfit)
        ...
        y_fit = UnDi.diffractionintensityInitialFit_X(zetaprime2,zetaxx,Ytran,listOfboots[Strpidx],AUndu.z0,WLofPixel,LambdaRfit,delta0fit+posdatas[listOfboots[Strpidx]]*.001,gammafit,Amplfit,bfit,zpfit,Yscreenfit,elevationfit,whichdata,cfit)
