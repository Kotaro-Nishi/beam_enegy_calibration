...
dispersion = 0.004808759252942803e-9
pixelof404nm = 1315.3024745554976
CalibWl = 404.6565e-9
...
def diffraction_dataset(params,i,yy):
    ...
    #Diff_intensity = UnDi.diffractionintensityInitialFit_X(yy,zetaxx,Ytran,i,AUndu.z0,WLofPixel,LambdaRfit,delta0fit+i*.001,gammafit,Amplfit,bfit,5.21,Yscreenfit,13.43,whichdata,.71) #460 #bfit
    #Diff_intensity = UnDi.diffractionintensityInitialFit_X(yy,zetaxx,Ytran,i,AUndu.z0,WLofPixel,LambdaRfit,delta0fit+i*.001,gammafit,Amplfit,bfit,5.22,Yscreenfit,13.677,whichdata,.68) #380 #bfit
    Diff_intensity = UnDi.diffractionintensityInitialFit_X(yy,zetaxx,Ytran,i,AUndu.z0,WLofPixel,LambdaRfit,delta0fit+i*.001,gammafit,Amplfit,0.,5.2,Yscreenfit,13.,whichdata,.67) #460
    #Diff_intensity = UnDi.diffractionintensityInitialFit_X(yy,zetaxx,Ytran,i,AUndu.z0,WLofPixel,LambdaRfit,delta0fit+i*.001,gammafit,Amplfit,0.,5.21,Yscreenfit,13.73,whichdata,cfit) #380
...
#for i_folder in range(len(FolderList)-5,len(FolderList)):
for i_folder in range(len(FolderList)-1,len(FolderList)):
    ...
    for Saves in range(0,1000):
        ...
        for ii in range(LengthOfInterval):
            ...
            stripe = nptp(np.array([stripejjj])*7000000.)
        ...
        Guessvalueslmfit = [3.8,-.0001,352.4,.190+.01+k*.001,.6,.6]
        ...
        for iy,y, in enumerate(dataBooted):
            ...
            #fit_params.add('bfit_%i' % (listOfboots[iy]),value=Guessvalueslmfit[4])
        ...
        for Strpidx in range(NumOfSeries):
            ...
            y_fitLst.append(UnDi.diffractionintensity(zetaprime2,Ytran,listOfboots[Strpidx],AUndu.z0,WLofPixel,LambdaRfit,delta0fit+listOfboots[Strpidx]*.001,gammafit,Amplfit,bfit,5.0207,Yscreenfit)[480])
        ...
        for Strpidx in range(NumOfSeries):
            if 1:#Strpidx > 600 and Strpidx > 610:
                ...
                y_fit = UnDi.diffractionintensity(zetaprime2,Ytran,listOfboots[Strpidx],AUndu.z0,WLofPixel,LambdaRfit,delta0fit+listOfboots[Strpidx]*.001,gammafit,Amplfit,bfit,5.0207,Yscreenfit)
        ...
        #amplcfitCorrel = result.covar[0][4] / (result.params["Amplfit_%i"% listOfboots[FirstSingularitem]].stderr*result.params["cfit_%i"% listOfboots[FirstSingularitem]].stderr)
        ...
        if DoNotSave == 0:
            ...
            """
            with open(DataPath + SaveSharedP + "\\amplcfitCorrel.csv", 'a+', newline='') as write_obj:
                # Create a writer object from csv module
                csv_writer = csv.writer(write_obj)
                # Add contents of list as last row in the csv file
                csv_writer.writerow([amplcfitCorrel])
            """
            """
