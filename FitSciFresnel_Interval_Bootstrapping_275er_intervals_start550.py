...
zetaxx = np.linspace(-.0,.0,3)
zetaxx = np.linspace(-.001412,.001412,3)
zetaxx = np.array([0.])
fixedsize = .245
...
def diffraction_dataset(params,i,yy):
    ...
    Diff_intensity = UnDi.diffractionintensityInitialFit_X(yy,zetaxx,Ytran,i,AUndu.z0,WLofPixel,LambdaRfit,delta0fit+i*.001,gammafit,Amplfit,bfit,5.458,Yscreenfit,13.,whichdata,1.25)#UnDi.diffractionintensity(yy,Ytran,i,AUndu.z0,WLofPixel,LambdaRfit,delta0fit+i*.001,gammafit,Amplfit,bfit,5.2,Yscreenfit)
...
#for i_folder in range(0,6):#len(FolderList)):
    ...
    SaveSharedP = FolderList[int(i_folder/10.)]#"\SharedP_20210401_100_300725_825_seed10000_new_zpfit"
    ...
    for Saves in range(0,1000):
        ...
        Guessvalueslmfit = [5.,.0001,352.4,.190+.01+k*.001,.6,1.3]#,3.e-4]
