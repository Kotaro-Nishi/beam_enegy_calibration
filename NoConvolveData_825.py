...
whichdata = 0 # Is NoConvolveData_825.py
...
for Saves in range(0,1):
    ...
    if 1:
        ...
        #cv2.imshow("resid",data)
        #cv2.waitKey(0)

        filename = 'DataImage.tif'
        cv2.imwrite(filename, data) 

        y_fitLst = []
        y_Sourcelst = []
        for Strpidx in range(int(NumOfSeries)):
            ...
            y_fitLst.append(UnDi.diffractionintensityInitialFit_X(zetaprime2,zetaxx,Ytran,listOfboots[Strpidx],AUndu.z0,WLofPixel,LambdaRfit,delta0fit+listOfboots[Strpidx]*.001,gammafit,Amplfit,bfit,zpfit,Yscreenfit,elevationfit,whichdata,cfit))

            Sourcelst = 10e-10*UnDi.plainintensity(zetaprime2,zetaxx,Ytran,listOfboots[Strpidx],AUndu.z0,WLofPixel,LambdaRfit,delta0fit+.001*listOfboots[Strpidx],gammafit,Amplfit,bfit,zpfit,Yscreenfit,elevationfit)
            y_Sourcelst.append(abs(np.array(Sourcelst[0])))

        y_fitLst = np.array(y_fitLst)

        #cv2.imshow("resid",y_fitLst)
        #cv2.waitKey(0)

        filename = 'FITImage.tif'
        cv2.imwrite(filename, y_fitLst.astype(np.float32))

        y_Sourcelst = np.array(y_Sourcelst)

        #cv2.imshow("resid",y_Sourcelst)
        #cv2.waitKey(0)

        filename = 'SourceImage.tif'
        cv2.imwrite(filename, y_Sourcelst.astype(np.float32))
