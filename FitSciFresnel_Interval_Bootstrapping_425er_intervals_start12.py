...
def diffraction_dataset(params,i,yy):
    ...
    Diff_intensity = UnDi.diffractionintensity(yy,Ytran,i,AUndu.z0,WLofPixel,LambdaRfit,delta0fit+i*.001,gammafit,Amplfit,bfit,5.2,Yscreenfit)
...
def objective_diffraction(params, yy, data,weights):
    ...
    for Strpidx in range(ndata):
        ...
        resid[Strpidx] = (diffractionresult - data[Strpidx])*(1./(np.array([1.5]*len(weights[0]))+(1./.2)*(1./2.)*np.sqrt(0.05+abs(diffractionresult)*.2/.7)))
...
for i_folder in range(len(FolderList)-5,len(FolderList)-1):
