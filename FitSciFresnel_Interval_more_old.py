...
import UndulatorDiffractionOneDimensional_more as UnDi
...
pixelof404nm = 1315.3590404676256
CalibWl = 404.6565e-9
...
while k < 3:#int(len(filelist)/4)-NumOfSeries:
    ...
    for ii in range(NumOfSeries):
        ...
        for i in range(4):
            ...
            PictureMinusBG = nptp(nptp(PictureMinusBG)[(chosenpx-WidthOfStripe):chosenpx])
            #print(len(PictureMinusBG[0]))
            PictureMinusBGForSigma = nptp(nptp(PictureMinusBGForSigma)[(chosenpx-WidthOfStripe):chosenpx])
    ...
    if 1:#Amplfit == -100:
        Guessvalueslmfit = [1.2,.0001,354.0,408.184e-9,.190+.01+k*.001,.6,4.4]#,3.e-4]
        #Guessvalueslmfit = [3.2,-.0001,351.0,.195+.01+k*.001,1.,3.,-3.e-4] mirrad
    else:
        Guessvalueslmfit = [Amplfit,Ytran,gammafit,LambdaRfit,delta0fit+kstep*.001,bfit,zpfit]#,Yscreenfit]
    ...
    for iy,y, in enumerate(data):
        ...
        fit_params.add('delta0_%i' % (iy+1),value=Guessvalueslmfit[4])#, min = Guessvalueslmfit[4]-.1, max=Guessvalueslmfit[4]+.1)

        fit_params.add('bfit_%i' % (iy+1),value=Guessvalueslmfit[5])#, min = Guessvalueslmfit[4]-.1, max=Guessvalueslmfit[4]+.1)
        fit_params.add('zpfit_%i' % (iy+1),value=Guessvalueslmfit[6])#, min = Guessvalueslmfit[4]-.1, max=Guessvalueslmfit[4]+.1)
