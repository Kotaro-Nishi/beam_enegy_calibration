...
def objective_diffraction(params, yy, data,weights):
    ...
    for Strpidx in range(ndata):
        ...
        #resid[Strpidx] = (diffractionresult - data[Strpidx])*(1./(np.array([5.5]*240+[5.2]*606+[5.5]*(1000-606-240))+np.sqrt(1/5.)*np.sqrt(1/4.)*(1./.2)*np.sqrt(0.05+abs(diffractionresult)*.2/.7)))#Result OK 20220704
...
whichdata = 0 # Is like NoConvolveData_825.py
...
for i_folder in range(len(FolderList)-1,len(FolderList)):
    ...
    for Saves in range(0,1000):
        ...
        Guessvalueslmfit = [8.4,.0001,352.4,0.1863,3.6,5.2,13.,10.06]#,.35e-4]
